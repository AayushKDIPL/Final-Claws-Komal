...
const SubcategoryController = {
  getSubCategories: async (req, res) => {
    try {
      ...
      const subCategories = await Subcategory.find({}); // Populate category field
  ...
  deleteSubCategory: async (req, res) => {
    try {
      let subcategory = await Subcategory.findById(req.params._id);
      if (!subcategory) throw new Error("Invalid call");
      await Subcategory.findByIdAndDelete(req.params._id);
      return res
        .status(200)
